...
<ParamField body="fieldList" type="json">
  리드의 이름, 금액, 소스를 제외한 필드값을 설정합니다. name 에는 field 의 이름을 입력합니다. field 의 유형이 텍스트나 단일 선택인 경우에는 stringValue 에 값을 설정하고, field 의 유형이 숫자인 경우에는 numberValue 에 값을 설정하고, field 의 유형이 날짜인 경우에는 dateValue 에 값을 설정합니다.
</ParamField>
...
<ParamField body="fieldList" type="json">
  리드와 연결된 고객의 이름과 이메일을 제외한 필드값을 설정합니다. name 에는 field 의 이름을 입력합니다. field 의 유형이 텍스트나 단일 선택인 경우에는 stringValue 에 값을 설정하고, field 의 유형이 숫자인 경우에는 numberValue 에 값을 설정하고, field 의 유형이 날짜인 경우에는 dateValue 에 값을 설정합니다.
</ParamField>
...
<ParamField body="organizationFieldList" type="json">
  리드와 연결된 회사의 이름을 제외한 필드값을 설정합니다. name 에는 field 의 이름을 입력합니다. field 의 유형이 텍스트나 단일 선택인 경우에는 stringValue 에 값을 설정하고, field 의 유형이 숫자인 경우에는 numberValue 에 값을 설정하고, field 의 유형이 날짜인 경우에는 dateValue 에 값을 설정합니다.
</ParamField>
